...
public class DriveAutoCappedSpeedSlowerRot2 extends CommandBase {
  ...
  public DriveAutoCappedSpeedSlowerRot2(Pose2d targetPosition, DriveSubsystem driveSubsystem) {
  ...
  public void execute() {
    ...
    xOutput = (Math.abs(xOutput) > maxSpeed)
        ? Math.signum(xOutput) * maxSpeed
        : xOutput;
    yOutput = (Math.abs(yOutput) > maxSpeed)
        ? Math.signum(yOutput) * maxSpeed
        : yOutput;
